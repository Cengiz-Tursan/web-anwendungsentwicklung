...
class ElementFactory extends Object {
	...
	createHtmlElement (localName, properties = {}, attributes = {}, ...styleClasses) {
		...
		if ((localName = localName.trim()).length === 0) throw new RangeError(localName);
	...
	createCoordinatesText (...coordinates) {
		if (!(coordinates instanceof Array)) throw new TypeError();
		if (coordinates.length % 2 !== 0) throw new RangeError();
